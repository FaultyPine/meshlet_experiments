...
static void buildTriangleAdjacency(TriangleAdjacency2& adjacency, const unsigned int* indices, size_t index_count, size_t vertex_count, meshopt_Allocator& allocator)
{
	...
	// each vertex in every triangle, count the number of times it appears
	// so vertices that are used in more than 1 triangle will have >1 count
	...
	// for every (unique) vertex, record an offset
	// which represents it's place in a buffer where duplicate vertices are allowed
	// i.e. if theres vertex (0,0,0), (0,0,0), (1,1,1), and (2,2,2)
	// indices would be like (0,0,2,3) and vertices would be (0,0,0),(1,1,1),(2,2,2)
	// this records offset in original non-de-deuplicated vertex buffer (?)
